...
def minimize_cut_cost(CUT_X, CUT_Y):
    ...
    def calc(min_x, max_x, min_y, max_y, CUT_X, CUT_Y):  # min_x, min_yは、CUT_X, CUT_Yのindexを与える
        ANS = 1 << 60
        ...
        if max_x > min_x+1:
            for i in range(min_x+1, max_x):
                tempscore = line_cut*400*400/(CUT_X[max_x]-CUT_X[min_x])/(
                    CUT_Y[max_y]-CUT_Y[min_y])+calc(min_x, i, min_y, max_y, CUT_X, CUT_Y)+calc(i, max_x, min_y, max_y, CUT_X, CUT_Y)
                if ANS > tempscore:
                    ANS = tempscore
        ...
        if max_y > min_y+1:
            for j in range(min_y+1, max_y):
                tempscore = line_cut*400*400/(CUT_X[max_x]-CUT_X[min_x])/(
                    CUT_Y[max_y]-CUT_Y[min_y])+calc(min_x, max_x, min_y, j, CUT_X, CUT_Y)+calc(min_x, max_x, j, max_y, CUT_X, CUT_Y)
                if ANS > tempscore:
                    ANS = tempscore
        ...
        if max_x > min_x+1 and max_y > min_y+1:
            for i in range(min_x+1, max_x):
                for j in range(min_y+1, max_y):
                    tempscore = point_cut*400*400/(CUT_X[max_x]-CUT_X[min_x])/(CUT_Y[max_y]-CUT_Y[min_y]) + calc(min_x, i, min_y, j, CUT_X, CUT_Y)+calc(
                        min_x, i, j, max_y, CUT_X, CUT_Y)+calc(i, max_x, min_y, j, CUT_X, CUT_Y)+calc(i, max_x, j, max_y, CUT_X, CUT_Y)
                    if ANS > tempscore:
                        ANS = tempscore
    ...
    def from_cut(block_id, min_x, max_x, min_y, max_y):  # min_x, min_yは、CUT_X, CUT_Yのindexを与える
        ...
        if (min_x, max_x, min_y, max_y) in FROM:
            ...
            if x == -1:
                ...
                cut_ans_order.append(
                    "cut"+"["+block_id+"]"+"["+str(CUT_X[x])+","+str(CUT_Y[y])+"]")
