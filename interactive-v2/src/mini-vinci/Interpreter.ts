...
export class Interpreter {
    ...
    interpret(
        ...
        switch (instruction.typ) {
            case InstructionType.NopInstructionType ||
                InstructionType.CommentInstructionType: {
            ...
            case InstructionType.ColorInstructionType: {
                return this.colorCanvas(
                    lineNumber,
                    context,
                    instruction as ColorInstruction
                );
            }
            case InstructionType.PointCutInstructionType: {
                return this.pointCutCanvas(
                    lineNumber,
                    context,
                    instruction as PointCutInstruction
                );
            }
            case InstructionType.VerticalCutInstructionType: {
                return this.verticalCutCanvas(
                    lineNumber,
                    context,
                    instruction as VerticalCutInstruction
                );
            }
            ...
            case InstructionType.SwapInstructionType: {
                return this.swapCanvas(
                    lineNumber,
                    context,
                    instruction as SwapInstruction
                );
            }
            case InstructionType.MergeInstructionType: {
                return this.mergeCanvas(
                    lineNumber,
                    context,
                    instruction as MergeInstruction
                );
            }
        }
    ...
    colorCanvas(
        ...
        if (!block) {
            throw Error(
                `At ${line}, encountered: Block Id of [${blockId}] is not found!`
            );
        }
        // TypeCheck Ends
    ...
    pointCutCanvas(
        ...
        if (block.typ === BlockType.ComplexBlockType) {
            ...
            (block as ComplexBlock).subBlocks.forEach((subBlock) => {
                ...
                // Case 7
                if (
                    subBlock.topRight.px <= point.px &&
                    subBlock.topRight.py <= point.py
                ) {
    ...
    verticalCutCanvas(
        ...
        if (
            !(
                block.bottomLeft.px <= lineNumber &&
                lineNumber <= block.topRight.px
            )
        ) {
            throw Error(
    ...
    horizontalCutCanvas(
        ...
        if (
            !(
                block.bottomLeft.py <= lineNumber &&
                lineNumber <= block.topRight.py
            )
        ) {
            throw Error(
                `At ${line}, encountered: Line number is out of the [${blockId}]! Block is from ${block.bottomLeft} to ${block.topRight}, point is at ${lineNumber}!`
    ...
    swapCanvas(
        ...
        if (!block1) {
            throw Error(
                `At ${line}, encountered: Block Id of [${blockId1}] is not found!`
            );
        }
        const block2 = context.blocks.get(blockId2);
        if (!block2) {
            throw Error(
                `At ${line}, encountered: Block Id of [${blockId2}] is not found!`
            );
        }
        // TypeCheck Ends
        ...
        // Processing Starts
        if (
            block1.size.px === block2.size.px &&
            block1.size.py === block2.size.py
        ) {
            ...
            throw Error(
                `At ${line}, encountered: Blocks are not the same size, [${blockId1}] has size [${block1.size.px},${block1.size.py}] while [${blockId2}] has size [${block2.size.px},${block2.size.py}]`
            );
        }
        // Processing Ends
