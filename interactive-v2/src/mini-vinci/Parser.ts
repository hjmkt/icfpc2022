    ...
    parseLine(lineNumber: number, line: string): ParseResult {
        ...
        if (colorMatchResult) {
            ...
            const color = new RGBA(
                colorMatchResult.slice(6).map((value) => +value) as [
                    number,
                    number,
                    number,
                    number
            ...
            return {
                typ: "instruction",
                result: { typ, blockId, color } as ColorInstruction,
            };
        }
        ...
        if (pointCutMatchResult) {
            ...
            return {
                typ: "instruction",
                result: { typ, blockId, point } as PointCutInstruction,
            };
        }
        ...
        if (mergeMatchResult) {
            ...
            return {
                typ: "instruction",
                result: { typ, blockId1, blockId2 } as MergeInstruction,
            };
        }
        ...
        if (swapMatchResult) {
            ...
            return {
                typ: "instruction",
                result: { typ, blockId1, blockId2 } as SwapInstruction,
            };
        }

        return {
            typ: "error",
            result: [lineNumber, `Cannot parse the instruction [${line}]!`],
        };
    }
}
