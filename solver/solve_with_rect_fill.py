...
from rect_fill import find_cand_rect, rect_to_moves
...
def solve(problem, seed, token, resume_moves):

    width = 400
    height = 400

    target_image = cv2.imread(f"./problems/images/{problem}.png", cv2.IMREAD_UNCHANGED)
    ...
    if problem >= 26 and problem <= 35:
        with open(f"./problems/json/{problem}.initial.json") as f:
            ...
            for row in range(rows):
                for col in range(cols - 1):
                    id0 = (
                        f"{row*cols}" if col == 0 else f"{rows*cols+row*(cols-1)+col-1}"
                    )
                    id1 = f"{row*cols+col+1}"
                    initial_moves.append(
                        Move("merge", {"block_id0": id0, "block_id1": id1})
                    )
            for row in range(rows - 1):
                id0 = (
                    f"{rows*cols-1+cols-1}"
                    if row == 0
                    else f"{rows*cols-1+rows*(cols-1)+row}"
                )
            ...
            for row in range(rows):
                for col in range(cols):
                    ...
                    canvas.blocks[block["blockId"]] = SimpleBlock(
                        x0, y0, x1 - x0, y1 - y0, block["color"], block["blockId"]
                    )
                    canvas.all_blocks[block["blockId"]] = SimpleBlock(
                        x0, y0, x1 - x0, y1 - y0, block["color"], block["blockId"]
                    )
                    canvas_for_cost.blocks[block["blockId"]] = SimpleBlock(
                        x0, y0, x1 - x0, y1 - y0, block["color"], block["blockId"]
                    )
                    canvas_for_cost.all_blocks[block["blockId"]] = SimpleBlock(
                        x0, y0, x1 - x0, y1 - y0, block["color"], block["blockId"]
                    )
    ...
    while True:
        (cand_rect, cand_score_diff) = find_cand_rect(
            canvas, target_image, seed, alternative_cost
        )
    ...
    if token is not None:
        ...
        submission_results = list(
            filter(lambda x: x["problem_id"] == problem, submission_results)
        )
        if len(submission_results) > 0:
            min_score = submission_results[0]["min_cost"]
            if score < min_score or min_score == 0:
                print(
                    f"post submission with score = {score}, updated from {min_score}",
                    file=sys.stderr,
                )
...
if __name__ == "__main__":
    ...
    solve(args.problem, args.seed, args.token, resume_moves)
