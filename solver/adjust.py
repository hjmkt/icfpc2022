...
def adjust(problem, seed, token, MOVES):
    ...
    def calc_rect_score(RECT):
        ...
        for x, y, width, height, color in RECT:
            costmin, hflag, m1flag, m2flag = cost_calc_fin(
                x, y, width, height, alternative_cost
            )
            moves = change_hmflag_to_move(
                canvas2.global_id, x, y, width, height, color, hflag, m1flag, m2flag
            )
            MOVES2 += moves
            for m in moves:
                canvas2.exec_move(m)
    ...
    solve(problem, seed, token, MOVES2)
...
if __name__ == "__main__":
    ...
    adjust(args.problem, args.seed, args.token, MOVES)
